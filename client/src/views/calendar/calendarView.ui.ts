...
export default class CalendarView extends UIComponent {
  ...
  private setCalendarOsBarControls() {
    ...
    for (let i = 0; i < 12; i++) {
      monthSelector.addOptionFull(DateText.month(i), i + "", (month) => {
        calendarView.createCalendar(
          new Date(calendarView.currentMonth.getFullYear(), month, 1),
        );
      });
    }
    ...
    for (
      ...
      yearSelector.addOption(i + "", (year) => {
        calendarView.createCalendar(
          new Date(year, calendarView.currentMonth.getMonth(), 1),
        );
      });
    }
